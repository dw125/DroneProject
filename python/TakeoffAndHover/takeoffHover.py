...
def set_rc_channel_pwm(mav_connection, channel_id, pwm=1500):
    ...
    rc_channel_values = [63556 for _ in range(18)]
...
def set_rc_channel_all(mav_connection, pwm=1500):

    rc_channel_values = [pwm for _ in range(18)]
    mav_connection.mav.rc_channels_override_send(
        mav_connection.target_system,                # target_system
        mav_connection.target_component,             # target_component
        *rc_channel_values)
...
def hover(mav_connection, duration):
	...
	for i in range(0, duration*10):
		set_rc_channel_all(mav_connection)
		time.sleep(0.1)
	print("end hover")
	#set_rc_channel_pwm(mav_connection, 3, pwm=0)
	return True

def hoverLoop(mav_connection, duration):
...
def main():
    ...
    arm(mav_connection)
    print("main: takeoff")
    takeoff(mav_connection, 1, wait_ready=False)
    time.sleep(5)
    hover(mav_connection, 30)
    print("main: land")
    land(mav_connection)
    time.sleep(3)
